...
- 右侧：把左侧涂鸦作为输入生成短视频。当前实现为“智能模式”：先用视觉模型描述涂鸦，再调用 `wan2.2-t2v-plus` 生成视频（wan 系列固定 5 秒）。如果上游返回 OSS 签名直链，前端直接播放/下载该直链，避免服务端 TLS 超时。
...
- 负责：
  ...
  - 上游给出视频直链时（尤其是阿里云 OSS 签名 URL），**不会在服务端转拉**（避免 Vercel/边缘网络 TLS handshake timeout），而是返回 `{ url }` 让前端浏览器直连获取。
- 关键防御：
...
功能点：
- “开始/下载/分享”
- 风格按钮：蜡笔/彩铅/水彩/漫画/纸偶/涂鸦（拼音+汉字标注）
...
参数：
- 视频模型：`wan2.2-t2v-plus`（智能模式）
- `seconds`：固定 `5`（AIHubMix 的 wan 视频模型使用 1–3s 通常会失败；当前 UI 不提供调时长）
- `size=832x480` 横屏 480p
...
- 生成时长目前是 `5s` 固定（wan 模型限制）。如需 1–3s，需要换支持可变时长的视频模型，或生成后做剪辑。
- “全屏”在部分 iOS Safari 上可能受限制：组件有 fallback 的最大化样式（`canvasStageMax`）。
- 轮询策略偏保守：遇到上游偶发返回 JSON/text 但仍带 `tid` 的情况，会继续等待而不是直接失败。
- 如果出现 `TLS handshake timeout`，一般是服务端拉 OSS 视频失败；当前已改为“把直链返回给前端播放/下载”规避。
